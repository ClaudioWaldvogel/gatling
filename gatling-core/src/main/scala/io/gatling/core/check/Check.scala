...
case class CheckBase[R, P, X](
  ...
  def check(response: R, session: Session)(implicit cache: mutable.Map[Any, Any]): Validation[CheckResult] = {
    ...
    for {
      ...
      matched <- validator(actual).mapError(message => {
        if (errorMsg.isDefined) {
          val value: Validation[String] = errorMsg.get.apply(session)
          if (value.isInstanceOf[Success[String]]) value.get else value.asInstanceOf[Failure].message
        } else {
          s"${extractor.name}.${extractor.arity}.${validator.name}, $message"
        }
      })
